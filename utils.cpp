...
double haversineDistance(double lat1, double lon1, double lat2, double lon2) {
    ...
    double a = pow(sin(dlat / 2), 2) +
               cos(degreesToRadians(lat1)) * cos(degreesToRadians(lat2)) * pow(sin(dlon / 2), 2);
...
int calculateTravelTime(double lat1, double lon1, double lat2, double lon2) {
    ...
    int timeInMinutes =
        std::ceil(timeInHours * 60); // converting hours to minutes and rounding up to next integer
